...
def interpolateBoundaries(Q,level):
    for lv in np.arange(level,-1,-1):
      Q[lv,:] = 2*Q[lv+1,:]  - Q[lv+2,:]
      Q[(-1-lv),:]= 2*Q[(-2-lv),:] - Q[(-3-lv),:]
      Q[:,lv] = 2*Q[:,1+lv]  - Q[:,2+lv]
      Q[:,(-1-lv)]= 2*Q[:,-2-lv] - Q[:,-3-lv]
    return Q
...
def computeVorticity(X,Y,U,V):
    ...
    def stencil_roll_n(q):
        return [
            ...
            for s in [(0,0),(-1,0),(1,0),(0,-1),(0,1),(-2,0),(2,0),(0,-2),(0,2)]
            #            p     w     e    s       n     ww    ee    ss     nn
        ]
    ...
    def grad_q(q):
       ...
       qp, qw, qe, qs, qn, qww, qee, qss, qnn = q_st
       dqdx= (qww-8*qw+8*qe-qee)/(12*dx)
       dqdy= (qss-8*qs+8*qn-qnn)/(12*dy)       
       return dqdx,dqdy
    dudx,dudy=grad_q(U)
    dvdx,dvdy=grad_q(V)
    Wz = dvdx-dudy
    Wz = np.array(Wz)
    Wz=interpolateBoundaries(Wz,1)  # interpolate indices 0 and 1
    return Wz

def computeDiv(X,Y,U,V):
    dx=X[1,0]-X[0,0]
    dy=Y[0,1]-Y[0,0]
    def stencil_roll_n(q):
        return [
    ...
    dU = dudx+dvdy
    dU = np.array(dU)
    dU=interpolateBoundaries(dU,1)  # interpolate indices 0 and 1
    return dU

def computePressureGradient(X,Y,P):
    dx=X[1,0]-X[0,0]
    dy=Y[0,1]-Y[0,0]
    def stencil_roll_n(q):
        return [
    ...
    dpdx = np.array(dpdx)
    dpdy = np.array(dpdy)
    dpdx=interpolateBoundaries(dpdx,1)
    dpdy=interpolateBoundaries(dpdy,1)
    return dpdx,dpdy
...
def upsampleField(Q,usx,usy,ust,nPad,nlvl,dsize):
    ...
    tmin = dsize[4]
    tmax = dsize[5]
    xmin2 = xmin - xp1/nx1 * (xmax-xmin)
    xmax2 = xmax + (nx2-nx1-xp1)/nx1 * (xmax-xmin)
    ymin2 = ymin - yp1/ny1 * (ymax-ymin)
    ymax2 = ymax + (ny2-ny1-yp1)/ny1 * (ymax-ymin)
    tmin2 = tmin - tp1/nt1 * (tmax-tmin)
    tmax2 = tmax + (nt2-nt1-tp1)/nt1 * (tmax-tmin)
