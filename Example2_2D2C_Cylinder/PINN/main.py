...
def parse_args():
    ...
    parser.add_argument('--dst', type=int, default=1, help="Down-sampling in t for low-res data")
    ...
    parser.add_argument('--tmax', type=int, default=1, help="maximum time")
    parser.add_argument('--xmin', type=int, default=0.5, help="minimum time")
    parser.add_argument('--xmax', type=int, default=3, help="maximum time")
    parser.add_argument('--ymin', type=int, default=-1, help="minimum time")
    parser.add_argument('--ymax', type=int, default=1, help="maximum time")
    parser.add_argument('--Ns', type=float,default=0.1, help="Gaussian noise S.D")
...
def plot_func(problem, state, epoch, frame, cbinfo=None):
    ...
    inputs = [DataHR.XX,DataHR.YY,DataHR.TT]
    ...
    #fxnn, = problem.domain.neural_net(state,'fx_net')(*inputs)
    #fynn, = problem.domain.neural_net(state,'fy_net')(*inputs)
    
    #def grad(f, *deriv):
    #    for i in range(len(deriv)):
    #        for _ in range(deriv[i]):
    #            f = tf.gradient(f, inputs[i])[0]
    #    return f
    #dudy = grad(unn,0,1,0)
    #dvdx = grad(vnn,1,0,0)
    ...
    for j in range(0,Data.XX.shape[2],dtp):
       ...
       p2=ax[1].pcolor(DataHR.XX[:,:,j],DataHR.YY[:,:,j],DataHR.UU[:,:,j],vmin=-1.5,vmax=1.5)
       p3=ax[2].pcolor(DataHR.XX[:,:,j],DataHR.YY[:,:,j],unn[:,:,j],vmin=-1.5,vmax=1.5)
       ...
       p2=ax[1].pcolor(DataHR.XX[:,:,j],DataHR.YY[:,:,j],DataHR.VV[:,:,j],vmin=-1,vmax=1)
       p3=ax[2].pcolor(DataHR.XX[:,:,j],DataHR.YY[:,:,j],vnn[:,:,j],vmin=-1,vmax=1)
       for j2 in range(len(ax)):
         ax[j2].set_xlim([args.xmin, args.xmax])
         ax[j2].set_ylim([args.ymin, args.ymax])
         ax[j2].set_aspect('equal', adjustable='box')
       ...
       p2=ax[1].pcolor(DataHR.XX[:,:,j],DataHR.YY[:,:,j],DataHR.FX[:,:,j],vmin=-3,vmax=3)
       p3=ax[2].pcolor(DataHR.XX[:,:,j],DataHR.YY[:,:,j],fxnn[:,:,j],vmin=-3,vmax=3)
       for j2 in range(len(ax)):
         ax[j2].set_xlim([args.xmin, args.xmax])
         ax[j2].set_ylim([args.ymin, args.ymax])
         ax[j2].set_aspect('equal', adjustable='box')
       ...
       p2=ax[1].pcolor(DataHR.XX[:,:,j],DataHR.YY[:,:,j],DataHR.FY[:,:,j],vmin=-3,vmax=3)
       p3=ax[2].pcolor(DataHR.XX[:,:,j],DataHR.YY[:,:,j],fynn[:,:,j],vmin=-3,vmax=3)
       for j2 in range(len(ax)):
         ax[j2].set_xlim([args.xmin, args.xmax])
         ax[j2].set_ylim([args.ymin, args.ymax])
         ax[j2].set_aspect('equal', adjustable='box')
       ...
       WZHR=fdt.computeVorticity(DataHR.XX[:,:,j],DataHR.YY[:,:,j],DataHR.UU[:,:,j],DataHR.VV[:,:,j])
       wzn=fdt.computeVorticity(DataHR.XX[:,:,j],DataHR.YY[:,:,j],unn[:,:,j],vnn[:,:,j])
       
       fig, ax = plt.subplots(nrows=1,ncols=3)
       p1=ax[0].pcolor(Data.XX[:,:,j],Data.YY[:,:,j],WZ,vmin=-10,vmax=10)
       p2=ax[1].pcolor(DataHR.XX[:,:,j],DataHR.YY[:,:,j],WZHR,vmin=-10,vmax=10)
       p3=ax[2].pcolor(DataHR.XX[:,:,j],DataHR.YY[:,:,j],wzn,vmin=-10,vmax=10)
       for j2 in range(len(ax)):
         ax[j2].set_xlim([args.xmin, args.xmax])
         ax[j2].set_ylim([args.ymin, args.ymax])
         ax[j2].set_aspect('equal', adjustable='box')
    ...
    for j in range(wzn.shape[2]):
       wzn[:,:,j] = fdt.computeVorticity(DataHR.XX[:,:,j],DataHR.YY[:,:,j],un[:,:,j],vn[:,:,j])
    
    # save output data
    mdic = {'x_sol':DataHR.XX, 'y_sol':DataHR.YY, 't_sol':DataHR.TT, 'u_sol':un, 'v_sol':vn, 'wz_sol':wzn, 'fx_sol':fxn, 'fy_sol':fyn}
...
def history_func(problem, state, epoch, history, cbinfo):
    ...
    eUtrue= np.sqrt(np.mean((unn - DataHR.UU)**2)) 
    eVtrue= np.sqrt(np.mean((vnn - DataHR.VV)**2)) 
    eFxtrue= np.sqrt(np.mean((vnn - DataHR.VV)**2)) 
    eFytrue= np.sqrt(np.mean((vnn - DataHR.VV)**2)) 
...
tmin = DataHR.TT[0,0,0]
tmax = DataHR.TT[0,0,-1]
